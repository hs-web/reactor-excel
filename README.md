...
- CSV 先把单个 `WritableCell` 编码到有界 staging `ByteBuf`：小 cell 聚合到 `bufferSize` 再发送，大 cell 按 demand 零拷贝切分；核心路径不使用可随数据量增长的 `byte[]` 中间缓冲，也不通过竞争锁等待信号。
- XLSX/POI 保留阻塞写出特性，通过有界 OutputStream 桥接限制最终序列化的在途数据；工作簿创建、cell 修改、最终写出和取消清理统一隔离到可配置的阻塞 Scheduler。
- 现有 `Flux<byte[]>` 和 `OutputStream` 入口保持兼容；`Flux<byte[]>` 只在末端执行复制和 `ByteBuf` 释放。

`ByteBuf` 在成功发送给下游后转移所有权；未交付的缓冲区会在取消或错误时由写出器释放。大 cell 使用 retained slice，最后一个 slice 释放前会保留整个父缓冲（最大为 `maxEncodedCellBytes`）；下游应及时释放，不要长期暂存小 slice，否则可能产生明显的内存放大。

CSV 背压以 `WritableCell` 为上游最小单元：下游没有 demand 时不会继续请求 cell，同一时刻最多存在一个尚未满足的源请求。一次下游 demand 可能顺序拉取多个小 cell，直到聚合块达到 `bufferSize` 或数据源结束；staging 与 aggregate 各自有界，不会预取多个 cell。为防止单个异常大 cell 绕过数据集级背压，响应式 CSV 默认限制单个 cell 的编码结果为 16 MiB，可通过 `MaxEncodedCellBytesOption.of(bytes)` 调整。每个发出的 `ByteBuf` 仍不超过调用方配置的 `bufferSize`。

CSV 只为 UTF-8、UTF-16BE/LE 和 UTF-32BE/LE 显式写入 BOM；`UTF-16` 使用字符编码器自身产生的 BOM，GBK 等非 Unicode 字符集不写 BOM。
...
`ExcelWriter.write(dataStream, options)` 的默认输出块是 64 KiB；`WriterOperator.writeBuffer/writeByteBufs` 为兼容既有行为保留 10 KiB 默认值。两者都可通过带 `bufferSize` 的重载显式指定。
...
- CSV 始终最多保留一个未满足的源请求；小 cell 聚合时 staging 与 aggregate 的在途容量不超过两个 256 字节缓冲，并在完成、异常或取消后释放。
- 超过单 cell 编码上限时立即失败并释放内部 `ByteBuf`；两条输出路径的并发 request/cancel 均不死锁、不泄漏。
- 上游错误保持为 `onError`，下游取消会传播到数据源，不能转换为正常完成。
- 压测输出 CSV 分块数和 rows/s，用于确认小 cell 聚合后没有退化为每行一个输出块。
